![Sisipan logo](./docs/logo.png)

# Sisipan sample 🎙️

Sisipan is a sample podcast app, built with [Jetpack Compose][compose]. The goal of the sample is to
...
Sisipan is still in the early stages of development, and as such only one screen has been created so far. However,
...
This is implemented in [`DynamicTheming.kt`](app/src/main/java/com/example/Sisipan/util/DynamicTheming.kt), which provides the `DynamicThemePrimaryColorsFromImage` composable, to automatically animate the theme colors based on the provided image URL, like so:
...
Underneath, [`DominantColorState`](app/src/main/java/com/example/Sisipan/util/DynamicTheming.kt) uses the [Coil][coil] library to fetch the artwork image 🖼️, and then [Palette][palette] to extract the dominant colors from the image 🎨.
...
Using the example of the home screen in the [`com.example.Sisipan.ui.home`](app/src/main/java/com/example/Sisipan/ui/home) package:
...
- __Home:__ [`com.example.Sisipan.ui.home`](app/src/main/java/com/example/Sisipan/ui/home)
- __Discover:__ [`com.example.Sisipan.ui.home.discover`](app/src/main/java/com/example/Sisipan/ui/home/discover)
- __Podcast Category:__ [`com.example.Sisipan.ui.category`](app/src/main/java/com/example/Sisipan/ui/home/category)

## Data

### Podcast data

The podcast data in this sample is dynamically fetched from a number of podcast RSS feeds, which are listed in [`Feeds.kt`](app/src/main/java/com/example/Sisipan/data/Feeds.kt). 
...
The [`PodcastRepository`][podcastrepo] class is responsible for handling the data fetching of all podcast information:
 ...
 - The parsed entities are then added to the local data stores: [`PodcastStore`][podcaststore], [`EpisodeStore`][epstore] &  [`CategoryStore`][catstore] for storage in the local [Room][room] [`SisipanDatabase`][db] database.

 ### Follow podcasts

 The sample allows users to 'follow' podcasts, which is implemented within the data layer in the [`PodcastFollowedEntry`](app/src/main/java/com/example/Sisipan/data/PodcastFollowedEntry.kt) entity class, and as functions in [PodcastStore][podcaststore]: `followPodcast()`, `unfollowPodcast()`.

 ### Date + time

 The sample uses the JDK 8 [date and time APIs](https://developer.android.com/reference/java/time/package-summary) through the [desugaring support][jdk8desugar] available in Android Gradle Plugin 4.0+. Relevant Room [`TypeConverters`](https://developer.android.com/reference/kotlin/androidx/room/TypeConverters) are implemented in [`DateTimeTypeConverters.kt`](app/src/main/java/com/example/sisipan/data/room/DateTimeTypeConverters.kt).
...
```
 ...
 [feeds]: app/src/main/java/com/example/sisipan/data/Feeds.kt
 [fetcher]: app/src/main/java/com/example/sisipan/data/PodcastFetcher.kt
 [podcastrepo]: app/src/main/java/com/example/sisipan/data/PodcastsRepository.kt
 [podcaststore]: app/src/main/java/com/example/sisipan/data/PodcastStore.kt
 [epstore]: app/src/main/java/com/example/sisipan/data/EpisodeStore.kt
 [catstore]: app/src/main/java/com/example/sisipan/data/CategoryStore.kt
 [db]: app/src/main/java/com/example/sisipan/data/room/SisipanDatabase.kt
 [homevm]: app/src/main/java/com/example/sisipan/ui/home/HomeViewModel.kt
 [homeui]: app/src/main/java/com/example/sisipan/ui/home/Home.kt
