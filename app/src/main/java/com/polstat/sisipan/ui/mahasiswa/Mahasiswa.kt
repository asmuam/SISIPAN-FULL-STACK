...
fun Mahasiswa(
    ...
    Surface(Modifier.fillMaxSize()) {
        MahasiswaContent(openDrawer,
            ...
            deleteMahasiswa = { viewModel.deleteMahasiswa(it.id) })
    }
}
...
fun MahasiswaContent(
    ...
    navigateToAddMahasiswa: () -> Unit,
    deleteMahasiswa: (MahasiswaCollection) -> Unit,
) {
    Log.i("REFRESH", "isRefreshing : ${isRefreshing}")
    ...
    Box(
                ...
                DynamicThemePrimaryColorsFromImage(dominantColorState) {
                    ...
                    LazyColumn(
                        ...
                        items(mahasiswaList) { mahasiswa ->
                            MahasiswaCard(mahasiswa, deleteMahasiswa = { deleteMahasiswa(it) })
                        }
                    }
                }
            }
        ...
        PullRefreshIndicator(
            refreshing = isRefreshing, state = state, modifier = Modifier.align(Alignment.TopCenter)
        )
    }
}
                ...
                Column {
                    ...
                    if (role != "MAHASISWA") {
                        Icon(
                            ...
                            tint = Color.Red,
                            modifier = Modifier
                                .clickable { deleteMahasiswa(userData) }
                                .padding(8.dp)
                                .size(24.dp) // Atur ukuran ikon
                        )
                    }
                }
...
@Composable
fun NameAndNIM(userData: MahasiswaCollection) {
...
@Composable
private fun Name(userData: MahasiswaCollection) {
    Text(
        text = userData.name,
        modifier = Modifier,
...
@Composable
fun MahasiswaProperty(label: String, value: Any) {
    Row(
        modifier = Modifier
            .fillMaxWidth()
            .padding(8.dp, 2.dp)
